...
Status: frozen design draft; no generated corpus or result exists yet.
...
The size may only change under a new protocol ID.
...
The split measures transfer of the fixed attribution contract to unseen
templates. It does not establish production-domain generalization, and the
open test labels are not suitable for a protected leaderboard.
...
9. every runner read is covered by the declared manifest;
...
- `trace_only`: 100/100 correct abstentions;
- `owner_probe`: 100/100 correct abstentions;
